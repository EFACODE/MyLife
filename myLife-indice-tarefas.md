...
| **T1.2** | 📝 | Append-only event store | Yes | `events` table + repository with append + read-by-stream; **no update/delete** at the repo layer; migration. `specs/domain/timeline/event-store.md` — **in review, awaiting approval.** |
