...
| **T2.3** | 📝 | Consent model + enforcement | **Yes** | `ConsentGranted`/`ConsentRevoked` events, granular per-integration consent, an enforcement dependency connectors must pass. `specs/domain/identity/consent.md` — **in review, awaiting approval.** |
