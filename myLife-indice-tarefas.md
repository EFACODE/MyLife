...
| **T4.7** | ✅ | Recurring bills (contas a pagar) | **Yes** | `BillRegistered`/`BillPaid`/`BillCancelled` events + `BillsService`; due occurrences derived read-time (no `BillDueGenerated`, no scheduler yet) by `BillsReportService`, filtering by period, account, paid/unpaid and overdue/not-overdue; authenticated `/finance/bills` endpoints; erasure updated. `specs/domain/finance/recurring-bills.md`. |
| **T4.8** | ✅ | Bill due-date alerts + outbound notifications | **Yes** | New `notifications` bounded context (stdlib SMTP + WhatsApp Cloud API channels; `NotificationRequested`/`NotificationSent`/`NotificationDeliveryFailed`; user preferences registry); Finance `BillAlertScanner`/`BillAlertsService` (due-soon 3/1/0-day + daily-while-overdue scan, one alert per bill); `POST /finance/bills/alerts/run`; first Celery beat schedule (`send_bill_reminders`, daily); erasure updated. `specs/domain/finance/bill-alerts.md`, `specs/domain/notifications/outbound-delivery.md`. |
| **T4.9** | ✅ | Open Finance connector (Pierre Finance aggregator) | **Yes** | `PierreFinanceConnector` (`source="openfinance"`) pulls already-aggregated accounts/transactions from the Pierre Finance API (no bank-connect endpoint exists there — the user links banks inside Pierre's own product and pastes an API key into My Life); auto-creates/links accounts, emits `OpenFinanceTransactionImported` + a `PositionValued(source="openfinance")` balance mark per sync, via the existing `T3.4` `ConnectorRunner`; new `CredentialVault` (Identity) for encrypted-at-rest third-party API keys; single `"openfinance"` consent scope on `T2.3`'s `ConsentService` (Pierre's key already scopes the whole connection — no per-institution token to scope separately); `POST/DELETE /finance/connectors/openfinance/credentials`, `POST /finance/connectors/openfinance/sync`. Pierre's `Transaction` JSON shape is undocumented upstream (its own OpenAPI leaves it `{}`) — the field mapping is a flagged, best-effort inference pending verification against a live sync. `specs/domain/finance/openfinance-connector.md`, `specs/domain/identity/third-party-credentials.md`. |
