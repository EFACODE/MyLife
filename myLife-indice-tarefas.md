...
| **T4.7** | ⬜ | Recurring bills (contas a pagar) | **Yes** | `BillRegistered`/`BillDueGenerated`/`BillPaid` events + `BillsService`; read-time `BillsReportService` filtering by period, account, paid/unpaid and overdue/not-overdue; authenticated `/finance/bills` endpoints. `specs/domain/finance/recurring-bills.md`. |
