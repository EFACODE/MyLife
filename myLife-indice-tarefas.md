...
| **T1.4** | 📝 | Raw ingestion store + provenance link | Yes | Separate `raw_records` table; normalized events reference `raw_record_id` + `source`. Keeps the external system authoritative. `specs/domain/timeline/raw-store.md` — **in review, awaiting approval.** |
