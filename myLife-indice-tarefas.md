...
| **T3.4** | 📝 | Connector framework & contract | **Yes** | Ports/adapters + idempotent sync job (via `T0.9` worker): pull → raw store → normalized events. The reusable ingestion spine. `specs/domain/timeline/connector-framework.md` — **in review, awaiting approval.** |
