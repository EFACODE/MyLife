...
| **T4.8** | ⬜ | Bill due-date alerts + outbound notifications | **Yes** | New `notifications` bounded context (SMTP email + WhatsApp Cloud API channels; `NotificationRequested`/`NotificationSent`/`NotificationDeliveryFailed`); Finance due/overdue bill scan; first Celery beat schedule (`send_bill_reminders`). `specs/domain/finance/bill-alerts.md`, `specs/domain/notifications/outbound-delivery.md`. |
